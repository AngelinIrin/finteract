...
public interface WorkingCapitalLoanTransactionRelationRepository
    ...
    @Query("""
            SELECT new org.apache.fineract.portfolio.workingcapitalloan.data.ChargeIdAndAmountHolder(r.toCharge.id, SUM(r.fromTransaction.transactionAmount))
            FROM WorkingCapitalLoanTransactionRelation r
            WHERE r.toCharge.loan.id = :loanId
              AND r.fromTransaction.transactionType = :transactionType
              AND r.fromTransaction.reversed = false
            GROUP BY r.toCharge.id
            """)
    List<ChargeIdAndAmountHolder> fetchTransactionAmountPerCharge(@Param("loanId") Long loanId,
