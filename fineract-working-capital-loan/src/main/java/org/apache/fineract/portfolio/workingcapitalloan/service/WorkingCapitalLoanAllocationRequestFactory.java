...
public class WorkingCapitalLoanAllocationRequestFactory {
    ...
    public WorkingCapitalLoanAllocationRequest build(final WorkingCapitalLoan loan, final WorkingCapitalLoanBalance balance,
        ...
        return build(loan, balance, charges, transactionDate, amount, null);
    }
    ...
    private List<WorkingCapitalPaymentAllocationType> resolveAllocationOrder(final WorkingCapitalLoan loan) {
        return resolveAllocationOrder(loan, null);
    }
    ...
    private List<WorkingCapitalPaymentAllocationType> resolveAllocationOrder(final WorkingCapitalLoan loan,
        ...
        if (rules != null && !rules.isEmpty()) {
            final PaymentAllocationTransactionType allocationType = mapTransactionType(transactionType);
            final WorkingCapitalLoanPaymentAllocationRule specificRule = allocationType != null ? findRule(rules, allocationType) : null;
            final WorkingCapitalLoanPaymentAllocationRule repaymentRule = specificRule != null ? specificRule
                    : findRule(rules, PaymentAllocationTransactionType.REPAYMENT);
    ...
    private PaymentAllocationTransactionType mapTransactionType(final LoanTransactionType type) {
        if (type == null) {
            return null;
        }
        for (PaymentAllocationTransactionType pat : PaymentAllocationTransactionType.values()) {
            if (type.equals(pat.getLoanTransactionType())) {
                return pat;
            }
        }
        return null;
